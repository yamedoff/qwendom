...
### Risk 1: Qwen Legacy models may not support tool-calling
**Mitigation**: 
- Test with `qwen3.7-plus` first (known tool support)
...
### Verification Steps
1. Run backend with `LLM_PROVIDER=qwen_legacy` and `QWEN_LEGACY_MODEL=qwen3.7-plus`
...
5. Test fallback: set `QWEN_LEGACY_MODEL=meta-llama/llama-3.3-70b-instruct` (may not support tools)
...
5. **Phase 6**: Multi-model routing (use Qwen 3.7 Plus for tool-heavy tasks, Qwen for text)
