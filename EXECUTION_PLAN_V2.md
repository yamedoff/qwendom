...
- OpenRouter provider support through Agno's OpenAI-compatible model class.
...
1. **Native tools are required**
   ...
   - Deterministic fallback may remain only for no-key local development mode, not for the serious OpenRouter path.
...
4. **Provider realism**
   - The active OpenRouter model must support tool calling.
...
flowchart TD
  ...
  AGENTS --> MODELS["OpenRouter / Qwen Cloud"]
...
3. Backend health reports OpenRouter model.
...
Check runtime risks with OpenRouter tool calling.
