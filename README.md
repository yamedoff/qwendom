...
flowchart LR
  ...
  AGNO --> QWEN["Qwen Cloud / OpenRouter (OpenAI-compatible API)"]
...
2. Set `LLM_PROVIDER` (`cerebras`, `qwen`, or `openrouter`) and the corresponding keys: `CEREBRAS_API_KEY`, `CEREBRAS_MODEL` for Cerebras; `QWEN_API_KEY`, `QWEN_BASE_URL`, `QWEN_MODEL` for Qwen Cloud; or `OPENROUTER_API_KEY`, `OPENROUTER_BASE_URL`, `OPENROUTER_MODEL` for OpenRouter. Also set `FRONTEND_ORIGIN`.
